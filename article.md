...
Arrow makes it easy to create date and time objects.
    ...
    date = arrow.get("2023-01-01 14:30", "YYYY-MM-DD HH:mm")
...
Arrow simplifies time zone conversions.
    ...
    local_time = utc_time.to('US/Central')
    print("UTC Time:", utc_time)
    print("Central Time:", local_time)
...
Arrow includes tools for working with time zones.
    ...
    timezones = arrow.now().format('ZZ')
    print("Time Zones:", timezones)

    # Convert using time zone name
    time_in_paris = arrow.now().to('Europe/Paris')
    print("Time in Paris:", time_in_paris)
...
Arrow allows easy manipulation of dates and times by adding or subtracting time.
    ...
    past = now.shift(years=-1, months=-1)
    print("Now:", now)
    print("Future:", future)
    print("Past:", past)
...
Example Output:
    ...
    Now: 2025-01-14T21:15:12.374752-06:00
    Past: 2024-12-07T16:15:12.374752-06:00
...
Example Output:
    ...
    Formatted Date: 2025-01-14 21:15:12
    Human Readable: just now
...
Arrow simplifies parsing and converting date strings.
    ...
    date_string = "14-Jan-2025"
    parsed_date = arrow.get(date_string, "DD-MMM-YYYY")
...
Arrow integrates seamlessly with Pandas, making it useful for time series data.
    ...
    # Convert using Arrow
    df['date'] = df['date'].apply(lambda x: arrow.get(x))
    df['date_string'] = df['date'].apply(lambda x: x.format('YYYY-MM-DD'))
    print(df)
...
You can easily handle time zone conversions in DataFrames using Arrow.
    ...
    df['date_central'] = df['date'].apply(lambda x: x.to('US/Central'))
    df['date_paris'] = df['date'].apply(lambda x: x.to('Europe/Paris'))
    print(df[['date_string', 'date_central', 'date_paris']])
...
Arrow supports operations on time intervals, such as checking overlaps or durations.
    ...
    start = arrow.get("2025-01-14 14:00", "YYYY-MM-DD HH:mm")
    end = arrow.get("2025-01-14 16:30", "YYYY-MM-DD HH:mm")
    duration = end - start
    print("Duration in minutes:", duration.total_seconds() / 60)
